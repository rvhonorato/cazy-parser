    ...
    for domain in species_domain_list:
        for initial in string.ascii_uppercase:
            ...
            index_list = re.findall(
                '"http://www.cazy.org/{}(\d.*).html" class="nav">(.*)</a>'.format(
                    species_domain_list
                ),
                species_list_hp,
            )
...
def fetch_families(main_link):
    ...
    family_list = [
        str(r.find("a")["href"].split("/")[-1].split(".html")[0]) for r in rows
    ]
...
def fetch_links(enzyme_class):
    ...
    for j, family in enumerate(family_list):
        ...
        family_soup = BeautifulSoup(
            urllib.request.urlopen(main_link), features="html.parser"
        )
        superfamily_list = [
            l.find("a")["href"]
            for l in family_soup.findAll("span", attrs={"class": "choix"})
        ][1:]

        superfamily_list = [
            f for f in superfamily_list if "structure" not in f
        ]  # remove structure tab for parsing
        for main_link in superfamily_list:
            ...
            if bool(page_index_list):
                ...
                for i in range(
                    ...
                    link = (
                        "http://www.cazy.org/"
                        + page_index_list[0]["href"].split("=")[0]
                        + "="
                        + str(i)
                    )
    ...
    for j, link in enumerate(link_list):
        ...
        for row in table.findAll("tr"):
            ...
            if len(tds) > 1 and tds[0].text != "Protein Name":
                ...
                try:
                    db_dic[protein_counter]["genbank"] = (
                        tds[3].find("a").text.replace("&nbsp;", "")
                    )  # get latest entry
                except AttributeError:
                ...
                try:
                    db_dic[protein_counter]["organism_code"] = species_dic[
                        tds[2].text.replace("&nbsp;", "")
                    ]
                except KeyError:
